...
#         metric_fc = metrics.AdaFace(512, opt.num_classes, m=0.4, s=30.)         #################### chaned from 1000 --> 500
...
#     onnx_model = onnx.load('/home/tawfik/git/AQUAFace/pretrained_models/R100_Glint360K.onnx')  # Update your path
...
# ===== UPDATED: Simplified distributed training setup =====
# Removed NCCL initialization - using Accelerator instead
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
def save_model(model, save_path, name, iter_cnt, last_loss, totalTrainLoss):
    """Save model checkpoint if loss improves"""
    save_name = os.path.join(save_path, name + '_' + str(iter_cnt) + '.pth')
    os.makedirs(save_path, exist_ok=True)
    
    print(f'Last Loss: {last_loss:.4f}, Current Loss: {totalTrainLoss:.4f}')
    
    if totalTrainLoss is not None and float(totalTrainLoss) < last_loss:
        last_loss = float(totalTrainLoss)
        torch.save(model.state_dict(), save_name)
...
def compute_cosine_similarity(features1, features2):
    # (Same as original)
    similarities = cosine_similarity(features1.detach().cpu().numpy(), features2.detach().cpu().numpy())
    return similarities.diagonal().reshape(-1, 1)
...
def cal_score(feature1, feature2, label, quality, age):
    ...
    pos_scores = np.array(data[1]) if 1 in data else np.empty((0, 4))
    neg_scores = np.array(data[0]) if 0 in data else np.empty((0, 4))
    
    return np.around(pos_scores, 4), np.around(neg_scores, 4)
...
class FDataset(Dataset):
    """
    Dataset class for AgeDB training pairs.
    Extracts person names (without numeric IDs) to group same person at different ages.
    """
    def __init__(self, data, transforms=None):
        super(FDataset, self).__init__()
        self.data = data
    ...
    def create_subject_id_map(self):
        """
        Create a dictionary that maps each unique person name to a unique integer.
        
        Filename format: <id>_<PersonName>_<age>_<gender>.jpg
        Example: 15664_KathrynGrayson_20_f.jpg
        
        Extraction: Gets only "KathrynGrayson" (index 1 after split by '_')
        """
        subject_ids = set()
        ...
        for sample in self.data:
            try:
                ...
                # Example: "dataset/AgeDB/15664_KathrynGrayson_20_f.jpg"
                # Split by '/' to get filename: "15664_KathrynGrayson_20_f.jpg"
                # Split by '_' to get parts: ["15664", "KathrynGrayson", "20", "f.jpg"]
                # Take index 1: "KathrynGrayson"
                
                filename1 = sample[0].split('/')[-1]  # Get filename from path
                person_name1 = filename1.split('_')[1]  # Get person name (index 1)
                ...
                print(f"Make sure filename format is: <id>_<PersonName>_<age>_<gender>.jpg")
                raise 
        
        # Create mapping: PersonName -> Integer ID (0 to N-1)
        subject_id_map = {name: idx for idx, name in enumerate(sorted(subject_ids))}
        
        print(f"Total unique identities (people) in dataset: {len(subject_id_map)}")
        print(f"Examples: {list(subject_id_map.items())[:5]}")
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            sample1: First image tensor
            sample2: Second image tensor
            label1: Identity ID of person in image 1
            label2: Identity ID of person in image 2
            label: 1 if same person, 0 if different
            quality: Quality level (0, 1, 2)
            age: Age difference
        """
        # Get paths and metadata
        image_path1 = self.data[idx][0]
        image_path2 = self.data[idx][1]
        label = int(self.data[idx][2])      # Same person (1) or different (0)
        quality = int(self.data[idx][3])    # Quality level
        age = int(self.data[idx][4])        # Age difference
        
        # Extract person names and get their IDs
...
def main():
    # ===== UPDATED: Load config with proper paths =====
    opt = Config()
    ...
    # ===== UPDATED: Load AgeDB training pairs =====
    print("\n" + "="*60)
    print("Loading AgeDB Training Data")
    print("="*60)
    print("USING TRAIN LIST FILE:", opt.train_list)

    data = np.loadtxt(opt.train_list, dtype=object)
    np.random.shuffle(data)
    print(f"Total pairs loaded: {data.shape[0]}")
    
    # Split: 99% train, 1% quick validation
    train_data, val_data = np.split(data, [int(0.99 * len(data))])
    print(f"Train pairs: {train_data.shape[0]}")
    print(f"Val pairs: {val_data.shape[0]}")
    ...
    if opt.metric == 'add_margin':
        ...
        metric_fc = metrics.AdaFace(512, opt.num_classes, m=0.4, s=30.)
    elif opt.metric == 'aquaface':
        metric_fc = metrics.Aquaface(512, opt.num_classes, m=0.4, s=30.)
    else:
        metric_fc = torch.nn.Linear(512, opt.num_classes)
    ...
    # ===== Freeze backbone, train only last layers =====
    print("\n" + "="*60)
    ...
    # ===== Training parameters =====
    steps = len(trainloader)  # NEW: Save once per epoch
    print(f"Will save model every {steps} iterations (once per epoch)")
    ...
    for epoch in range(opt.max_epoch):
        ...
        for ii, data in enumerate(tqdm(trainloader, desc=f"Epoch {epoch+1}/{opt.max_epoch}")):
            image1, image2, label1, label2, label, quality, age = data
            optimizer.zero_grad()
            ...
            # Every 'steps' iterations: compute GMM + contrastive loss
            if (iters + 1) % steps == 0:
                # Fit GMMs
                pos_X = np.concatenate(pos)
                neg_X = np.concatenate(neg)
                ...
                llr = pdf1 / pdf2
                llr = torch.tensor(llr)
                lr_norms = torch.clip(llr, min=-1, max=1)  # Stability
                lr_norms = lr_norms.to(device)
                ...
                # Print stats
                print(f'\nIter {iters+1}')
                print(f'Train Loss: {totalTrainLoss / steps:.4f}')
                ...
                # Regular step: only classification loss
                totalTrainLoss += cl_loss.item()
                
                outputs_np = outputs.data.cpu().numpy()
                outputs_np = np.argmax(outputs_np, axis=1)
                labels_np = labels.data.cpu().numpy()
                acc = np.mean((outputs_np == labels_np).astype(int))
                totalAcc += acc
        ...
        print(f'Train Loss: {totalTrainLoss / len(trainloader):.4f}')
        print(f'Train Acc: {100.0 * (totalAcc / len(trainloader)):.2f}%')
        
        # Save epoch checkpoint
        save_name = os.path.join(opt.checkpoints_path, f'{opt.backbone}_epoch_{epoch+1}.pth')
        torch.save(model.state_dict(), save_name)
        print(f'✅ Epoch checkpoint saved to {save_name}')
